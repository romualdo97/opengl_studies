...
int main(void)
{
	...
	*/
	...
	// Material related
	int uMaterialAmbientLoc = glGetUniformLocation(myShader.ID, "uMaterial.ambient");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightColor.z = 0.5 * sin(glfwGetTime() * 1.3f) + 0.5;
		...
		// pass material info
		glUniform3f(uMaterialAmbientLoc, 1.0f, 0.5f, 0.31f);
		glUniform3f(uMaterialDiffuseLoc, 1.0f, 0.5f, 0.31f);
		glUniform3f(uMaterialSpecularLoc, 0.5f, 0.5f, 0.5f);
