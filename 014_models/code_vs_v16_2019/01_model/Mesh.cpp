...
void Mesh::setupMesh()
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * indices.size(), &indices[0], GL_STATIC_DRAW); // copy data from CPU to GPU
	...
	// Total sent data
	std::cout << "Total mesh bytes sent to GPU:\n";
	std::cout << "sizeof(Vertex) * vertices.size() : " << sizeof(Vertex) * vertices.size() << std::endl;
	std::cout << "sizeof(unsigned int) * indices.size() : " << sizeof(unsigned int) * indices.size() << std::endl << std::endl;
}
